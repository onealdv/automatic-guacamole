...
async def run(r):
    ...
    print(f"Check HTML LIST has length {len(htmllist)}")
    for i in range(len(htmllist)):
        if htmllist[i] != '':
            ...
            try:
                ...
                for entry in ready1:
                    print(f"Found {entry['quantity']} ready size {entry['size']} {entry['price']}")
        ...
        # variantweight = ['kg']
            ...
            taglist = [taginput]
            print(f"Looking for SG URL {url2}")
        ...
        #print(f"Failed to export {urllist[i]}")
        # print(dlist)
    #     except:
