...
#WEB SCRAPING
linkorurl = input("Enter '0' if fixed price: ")
...
pricelist = []
emptylist = []
titlelist = ['[PO] ' + g_name.text]
bodylist = ['<meta charset="utf-8" /><div class="title"><meta charset="utf-8" /><div class="Trust__container__3bA3U" data-reactid="115"><div class="Trust__image-container__3yIDD" data-reactid="116"><strong>PRE RELEASE PRE ORDER</strong></div><div class="Trust__image-container__3yIDD" data-reactid="116">Product ini adalah <em>Pre Release Pre Order</em> untuk limited release '+ g_name.text +'. </div><div class="Trust__image-container__3yIDD" data-reactid="116"><ul><li>Pembayaran: Dikarenakan slot untuk PO ini sangat sedikit, kami membutuhkan<strong> DP minimal 50%.</strong></li><li>ETA: Sneaker akan sampai kepada anda 3-4 minggu setelah <em>release date</em> dari sneaker tersebut.</li><li>Cara pembelian: Pilih size dan click "Add to Cart". Kemudian ikuti instruksi untuk menyelesaikan transaksi.</li></ul></div><div class="Trust__image-container__3yIDD" data-reactid="116" style="text-align: left;"></div><div class="Trust__image-container__3yIDD" data-reactid="116"><img src="https://www.goat.com/images/products/icon_verification@2x.png" width="30" height="30" class="Trust__image__3gb_e" data-reactid="117" /></div><div class="Trust__title__3cC5B" data-reactid="118"><strong>1000% AUTHENTIC</strong></div><div class="Trust__desc__32B5p" data-reactid="119"><p class="p1">Semua sneaker dari VTL melewati proses "legit check" oleh tim profesional untuk memastikan keaslian sneaker tersebut.</p></div></div><div class="Trust__container__3bA3U" data-reactid="120"><div class="Trust__image-container__3yIDD" data-reactid="121"><img src="https://www.goat.com/images/products/icon_return@2x.png" width="30" height="30" class="Trust__image__3gb_e" data-reactid="122" /></div><div class="Trust__title__3cC5B" data-reactid="123"><strong>RETURNS</strong></div><div class="Trust__desc__32B5p" data-reactid="124">Kami tidak menerima pengembalian barang untuk product pre order.</div></div></div>']
vendorlist = ['Vendor']
taglist = [taginput]
publishlist = ['TRUE']
option1name = ['Size']
...
for result in g_size:
    ...
    if price != 'Bid' and size != "All" and price != '' and size != '': #excludes empty price and "all" size entry
        ...
        if linkorurl!='0':
            try:
                price = int(price)
                idr = (price+55)*1.08*13500 #format price + formula
                idr = roundup(idr)
            ...
            idr = int(price)
        # elif int(price) > 300 and linkorurl!=0:
        #     try:
        #         price = int(price)
        #         idr = (price+55)*1.1*13500 #format price + formula
        #         idr = roundup(idr)
